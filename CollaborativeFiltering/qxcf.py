...
def input2numlist(list0,list1,inputip="10.160.170.28",id=119):
	...
	# print(big_matrix)
	user_similarity = cosine_similarity(big_matrix)
	...
	print("用户最值得推荐的5个链接的编号："+str(favorite))
	print("")
	print("")
	print("链接名及权重如下：")
	print("")
	...
	for x in xrange(1,len(favorite)):
		...
		if item_id_dict_keylist[pos] not in ('text',u'首页',u'\n\t\t\t\t\t\t\t登录\n\t\t\t\t\t\t'):
			...
			returnList.append(item_id_dict_keylist[pos])
...
if __name__ == '__main__':
	...
	user_similarity = input2numlist(list0,list1,"10.158.46.179")
