...
def cosine_similarity(two_axis_list):
	...
	for x in range(listlen):
		...
		for y in range(t,listlen):
			...
			# print(str(x)+" & "+str(y)+" : "+str(return_two_axis_list[x][y]))
...
def writelist(listname):#二维数组
	maintext = ""
	for x in range(len(listname)):
		for y in range(len(listname[0])):
			maintext += str(listname[x][y]) + " "
		maintext += "\n\n"



	fwrite = open("maintable.txt","w")#如果已有文件，则覆盖
...
if __name__ == '__main__':
	...
	# for x in range(len(cos_list)):
	# 	for y in range(len(cos_list[0])):
	# 		maintext += str(cos_list[x][y]) + " "
	# 	maintext += "\n\n"
