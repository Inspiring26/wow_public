...
def kernelTrans(X, A, kTup): #核函数，输入参数,X:支持向量的特征树；A：某一行特征数据；kTup：('lin',k1)核函数的类型和参数
    m,n = shape(X)
    K = mat(zeros((m,1))) #zeros、mat都是numpy的方法，前者是创建一个m*1的0数组，后者是将前者转化为矩阵
    if kTup[0]=='lin': #线性函数
        K = X * A.T
...
def svmOneVsOne(dataArr,labelArr):
	b,alphas = smoP(dataArr, labelArr, 200, 0.0001, 10000, ('rbf', 1.3)) #通过SMO算法得到b和alpha
	datMat=mat(dataArr)
	labelMat = mat(labelArr).transpose()
	svInd=nonzero(alphas)[0]  #选取不为0数据的行数（也就是支持向量）
	sVs=datMat[svInd] #支持向量的特征数据
	labelSV = labelMat[svInd] #支持向量的类别（1或-1）
	return sVs,labelSV,alphas,svInd,b
def predictSvm(inlist,sVs,labelSV,alphas,svInd,b):
	datMat_test=mat(inlist)
	kernelEval = kernelTrans(sVs,datMat_test,('rbf', 1.3))
	predict=kernelEval.T * multiply(labelSV,alphas[svInd]) + b
	return sign(predict)
def mulClassify(inlist, inlabel):
	num=len(set(inlabel))
	for i in range(num):
		pass
...
if __name__ == '__main__':
	...
	fl = 0
	tol = 0
	for _ in range(type_n):
		tmp_list = []
		for x in range(len(in_y)):
			if in_y[x]==fl:
				tmp_list.append(in_x[x])
				tol += 1
		fl += 1
		data_size_list.append(tol)
		tol = 0
		data_table.append(tmp_list)

	# print(np.array(data_table[0]))
	# print(data_size_list)
	coef_table = [([0] * type_n) for i in range(type_n)]
	for x in range(type_n):
		for y in range(x+1,type_n):
			print(x,y)
			labelArr = np.array([-1]*data_size_list[x]+[1]*data_size_list[y])
			dataArr = np.array(data_table[x]+data_table[y])
			# print(labelArr)
			# print(dataArr)
			# print(svmOneVsOne(dataArr,labelArr))
			sVs,labelSV,alphas,svInd,b = svmOneVsOne(dataArr,labelArr)

			coef_table[x][y]= [sVs,labelSV,alphas,svInd,b]
	# print(coef_table[0][1][0])



	# 测试数据
	inlist=[5.4, 3. , 4.5, 1.5]
	# 初始值为0的类别列表
	type_list=[0]*type_n
	for x in range(type_n):
		for y in range(x+1,type_n):
			sVs,labelSV,alphas,svInd,b=coef_table[x][y]
			res_=predictSvm(inlist,sVs,labelSV,alphas,svInd,b)
	...
	# 分类结果
	print("分类结果:")
	print(type_list.index(max(type_list)))
